...
class CamInput:
    def __init__(self):
        ...
        # Load known faces from your trained data
        self.known_face_encodings, self.known_face_names = self.load_trained_model()

        # DataFrame to store recognized names
        self.present_df = pd.DataFrame(columns=['Name', 'Timestamp'])

        self.face_detection_started = False  # Flag to track face detection status
                    ...
                    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
                        ...
                        if True in matches:
                            ...
                            if name not in self.present_df['Name'].values:
                                ...
                                new_data = {'Name': [name], 'Timestamp': [timestamp_str]}
...
def stop():
    ...
    # Get current date for naming the CSV file
    current_date_str = pd.Timestamp.now().strftime('%Y-%m-%d')
    csv_filename = f'attendance {subjName} {current_date_str}.csv'

    # Save DataFrame to CSV
    cam_obj.present_df.to_csv(csv_filename, index=False)

    return f"CSV data saved to {csv_filename}"
